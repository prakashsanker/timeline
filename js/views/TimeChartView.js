...
app.TimeChartView = Backbone.View.extend({
	...
	initialize: function(){
		...
		var y = d3.scale.linear().domain([maxVal["quantity-sold"], minVal["quantity-sold"]]).range([0,height]);
		...
		var line = d3.svg.line()
			...
			.x(function(d) { 
				...
				return xScale(date)})
			.y(function(d) { 
				var quantitySold = y(d["quantity-sold"]);

				return quantitySold});

		svg.append("path")
			.attr("d", line(dataSortedByQuantitySold));
